...
def calculate_win_probabilities(num_trials: int = 100000, n_decks: int = 1) -> dict:
    ...
    Args:
    num_trials: Number of trials to run for each sequence pair.
    n_decks: The number of shuffled decks to generate for each trial.
    
    Returns:
    A dictionary of win/loss/draw probabilities for each sequence pair.
    """
    # Sequence pairs to simulate
    sequence_pairs = [('BBB', 'RBB'), ('BBR', 'RBB'), ('BRB', 'BBR'), ('BRR', 'BBR'),
                      ('RBB', 'RRB'), ('RBR', 'RRB'), ('RRB', 'BRR'), ('RRR', 'BRR')]

    probabilities = {}
    for p1_seq, p2_seq in sequence_pairs:
