...
def create_heatmaps(heatmap_total, heatmap_tricks, output_file, n_decks, draws_total, draws_tricks
                    ) -> None:
    """
    ...
    ax1 = sns.heatmap(heatmap_total * 100, annot=False, cmap='Blues',
    ...
    for i in range(8):
        for j in range(8):
            win_prob = heatmap_total[i, j]
            draw_prob = draws_total[i, j]
...
def fill_heatmaps(seed: int, 
    ...
    heatmap_total = np.zeros((len(all_sequences), len(all_sequences)))  
    heatmap_tricks = np.zeros((len(all_sequences), len(all_sequences)))

    # For draws
    draws_total = np.zeros((len(all_sequences), len(all_sequences)))  
            ...
            if result is not None:
                ...
                if 'win' in tricks_result and 'win' in cards_result:
                    # Store the probabilities in the heatmaps
                     ...
                     draws_total[i, j] = cards_result['draw'] 


    # Save the heatmaps
    create_heatmaps(heatmap_total, heatmap_tricks, output_file, n_decks + augment_decks, draws_total, draws_tricks)